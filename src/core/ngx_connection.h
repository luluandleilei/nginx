...
struct ngx_connection_s {
    void               *data;   //连接未使用时，data成员用于充当连接池中空闲连接链表中的next指针。
    ...
    ngx_pool_t         *pool;   //内存池。 一般在accept一个新连接时会创建一个内存池，而在这个连接结束时会销毁内存池。
    ...
    ngx_buf_t          *buffer; //用于接收、缓存客户端发来的字节流，每个事件消费模块可自由决定从连接池中分配多大的空间给该字段。
    ...
    unsigned            buffered:8; //缓存中的业务类型。 任何事件消费模块都可以自定义需要的标志位。这个buffered字段有8位， 最多可以同时表示8个不同的业务。 
    ...
    unsigned            log_error:3;    //本连接记录日志时的级别, 由ngx_connection_log_error_e枚举表示  
    unsigned            timedout:1;     //标志位，为 1时表示连接已超时
    unsigned            error:1;        //标志位，为 1时表示连接处理过程中出现错误
    unsigned            destroyed:1;    //标志位，为 1时表示连接已经销毁。这里的连接指的是TCP连接，而不是ngx_connection_t结构体。
    ...
    unsigned            sndlowat:1;     //标志位，为 1时表示只有在连接套接字对应的发送缓冲区必须满足最低设置的大小阈值时，事件驱动模块才会分发该事件。
                                        //与ngx_handle_write_event函数中的lowat参数是对应的。是否设置该连接的发送低水位标志，若设置过将不再重复设置
    unsigned            tcp_nodelay:2;  //标志位，表示如何使用TCP的nodelay特性。 它的取值范围是ngx_connection_tcp_nodelay_e枚举类型
    unsigned            tcp_nopush:2;   //标志位，表示如何使用TCP的nopush特性。 它的取值范围是ngx_connection_tcp_nopush_e枚举类型
