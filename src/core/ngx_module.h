...
//描述一个Nginx模块, Nginx模块基本接口
struct ngx_module_s {
    ngx_uint_t            ctx_index;	//当前模块在同类模块中的序号。这个成员常常是由管理这类模块的一个Nginx核心模块设置的。
    									//Nginx的模块化设计非常依赖于各个模块的顺序，ctx_index既用于表达优先级，也用于表明每个模块的位置，
    									//借以帮助Nginx框架快速获得某个模块的数据
    ngx_uint_t            index;		//当前模块在所有模块(ngx_modules数组)中的序号。Nginx启动时会根据ngx_modules数组设置各模块的index值
    ...
    void                 *ctx;			//指向特定类型模块的公共接口，例如核心模块的公共接口是ngx_core_module_t结构体，而事件模块的公共接口是ngx_event_module_t结构体
    									//HTTP模块的公共接口是ngx_http_module_t结构体
    ngx_command_t        *commands;		//模块指令，用于定义模块的配置文件参数
    ngx_uint_t            type;			//模块类型，它与ctx指针是紧密相关的。
    									//官方Nginx定义的模块类型: NGX_CONF_MODULE | NGX_CORE_MODULE | NGX_EVENT_MODULE | NGX_HTTP_MODULE | NGX_MAIL_MODULE | NGX_STREAM_MODULE
    									//实际上，还可以自定义新的模块类型

    ngx_int_t           (*init_master)(ngx_log_t *log);		//当在master进程启动时回调init_master，但到目前位置，框架代码从来不会调用它，因此，可将init_master设为NULL

    ngx_int_t           (*init_module)(ngx_cycle_t *cycle);	//init_module回调方法在初始化所有模块时被调用。在master/worker模式下，这个阶段将在启动worker子进程前完成

    ngx_int_t           (*init_process)(ngx_cycle_t *cycle); //init_process回调方法在正常服务前被调用。在master/worker模式下，在每个worker进程的初始化过程中会调用所有模块的init_process函数
    ngx_int_t           (*init_thread)(ngx_cycle_t *cycle);	//由于Nginx暂不支持多线程模式，所以init_thread在框架代码中没有被调用过，设为NULL
    void                (*exit_thread)(ngx_cycle_t *cycle);	//同上，设为NULL
    void                (*exit_process)(ngx_cycle_t *cycle); //exit_process回调方法在服务停止前被调用。在master/worker模式下，worker进程会在退出前调用所有模块的init_process函数

    void                (*exit_master)(ngx_cycle_t *cycle);	//exit_master回调方法将在master进程退出前被调用

    uintptr_t             spare_hook0;	//所有8个spare_hook变量也是保留字段，目前没有使用，可以使用NGX_MODULE_V1_PADDING宏来填充
